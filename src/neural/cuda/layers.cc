...
static float stdDev(float arr[], int n) {
  ...
  float var = 0.0f;             //initialize variance
  for (int i = 0; i < n; i++) {
    var += pow((arr[i] - m), 2); //add the squared difference from mean
  }
  var /= n;                     //divide by the number of elements
...
AttentionPolicyHead<DataType>::AttentionPolicyHead(
  ...
  for (const auto& enc : head.pol_encoder) {
    EncoderBlock<DataType>* pW = new EncoderBlock<DataType>(
        ...
        nullptr, 0, // smolgen weights not implemented in policy encoder heads yet.
        max_batch_size, ACTIVATION_SWISH,
        act_, true,
        1e-6);  // smolgen weights not implemented in policy encoder heads yet.
...
void EncoderBlock<DataType>::Eval(int N, DataType* in_out_tensor,
  ...
  */
  ...
  LayerNorm<DataType>(N * 64, embedding_op_size_, scratch, buffer1, mha_dense_b,
                      in_out_tensor, ln1_gammas, ln1_betas, default_eps_, alpha_,
